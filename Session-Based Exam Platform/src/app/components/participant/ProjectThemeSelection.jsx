...
export default function ProjectThemeSelection() {
    ...
    useEffect(() => {
        ...
        if (!session.examTheme && session.status !== 'project-theme') {
            navigate('/participant/theme-selection', { replace: true });
            return;
        }
        if (session.projectTheme) {
            setSelectedTheme(session.projectTheme);
        }
        apiGet('/api/project-themes')
            .then(async (data) => {
                if (data.length === 0) {
                    updateSession({ projectTheme: '', status: 'completed' });
    ...
    const handleConfirm = async () => {
        ...
        if (selectedTheme) {
            ...
            const selectedDuration = selectedThemeData?.durationMinutes || 120;
            const config = await apiGet('/api/config').catch(() => ({ waitingRoomEnabled: false }));
            const nextStatus = config.waitingRoomEnabled ? 'waiting-project' : 'project';
            updateSession({ projectTheme: selectedTheme, projectDurationMinutes: selectedDuration, status: nextStatus, waitingFor: config.waitingRoomEnabled ? 'project' : '' });
            if (session?.participantId) {
                await apiPatch(`/api/participants/${session.participantId}`, {
                    projectTheme: selectedTheme,
                    status: nextStatus,
                });
            }
            navigate(config.waitingRoomEnabled ? '/participant/waiting' : '/participant/project', { replace: true });
        }
    };
    ...
    return (<div className="min-h-screen overflow-hidden bg-gradient-to-br from-[#d9ecff] via-[#cfe7ff] to-[#b9dafb] px-4 py-4 text-slate-900 sm:px-6 lg:px-8">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute bottom-16 right-1/4 font-mono text-6xl font-semibold leading-none text-[#1e5ba8]/10">&lt;/&gt;</div>
      ...
      <div className="relative mx-auto max-w-5xl">
        <Paper elevation={0} sx={{
          ...
          <div className="relative overflow-hidden rounded-[16px]">
            ...
            <div className="absolute bottom-3 right-28 font-mono text-3xl font-semibold text-[#1e5ba8]/10">&lt;project&gt;</div>
            <div className="flex flex-col gap-3 p-4 pl-6 sm:flex-row sm:items-center sm:justify-between sm:p-5 sm:pl-7">
              <div>
                ...
                <h1 className="text-2xl font-semibold tracking-normal text-slate-950 sm:text-3xl">Select Your Project Theme</h1>
                <p className="mt-1.5 text-sm leading-6 text-slate-600">
                  Choose a project theme. Project themes are separate from quiz themes.
                </p>
              </div>

              <div className="flex shrink-0 items-center gap-3 rounded-md border border-[#1e5ba8]/20 bg-[#e9f4ff]/90 px-3 py-2 shadow-sm">
                <div className={`flex h-9 w-9 items-center justify-center rounded-md ${selectedTheme ? 'bg-[#1e5ba8] text-white' : 'bg-blue-100 text-[#1e5ba8]'}`}>
                  <Code2 className="h-4 w-4"/>
                </div>
                <div className="min-w-0">
                  <p className="max-w-[220px] truncate text-sm font-semibold text-slate-950">
                    {selectedThemeData ? selectedThemeData.name : 'No project theme selected'}
                  </p>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          {themes.map((theme) => {
              ...
              return (<Paper key={theme.id} elevation={isSelected ? 6 : 1} onClick={() => setSelectedTheme(theme.id)} sx={{
                borderRadius: '14px',
                cursor: 'pointer',
                ...
                <div className="absolute bottom-12 right-16 font-mono text-[11px] font-semibold text-[#1e5ba8]/22">
                  {isSelected ? 'project.lock()' : 'choose(project)'}
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 pr-8">
                    ...
                    <p className="mt-1 text-xs font-semibold text-[#1e5ba8]">{theme.durationMinutes || 120} menit</p>
                  </div>
                </div>
