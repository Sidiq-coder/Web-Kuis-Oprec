...
export async function createQuestion(req, res) {
    const { themeId, type, question, options, correctAnswer, correctAnswers, weight } = req.body;
    ...
    const id = `q-${Date.now()}`;
    const [created] = await query(`INSERT INTO questions (id, theme_id, type, question, options, correct_answer, correct_answers, weight)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
     RETURNING id, theme_id AS "themeId", type, question, options, correct_answer AS "correctAnswer", correct_answers AS "correctAnswers", weight`, [id, themeId, type, question, options ? JSON.stringify(options) : null, type === 'multiple-choice' ? correctAnswer ?? null : null, type === 'multiple-answer' ? JSON.stringify(normalizedCorrectAnswers) : null, weight]);
